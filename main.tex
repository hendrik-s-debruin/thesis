\documentclass[]{report}

	\usepackage{ifdraft}
	\usepackage[obeyFinal]{todonotes}

	\usepackage{titlesec}
	\setcounter{secnumdepth}{3}

	\usepackage{mathrsfs}
	\usepackage{amsfonts}
	\usepackage{amssymb}
	\usepackage{dsfont}
	\usepackage{amsmath}
		\usepackage{cases}
		%\usepackage[overload]{empheq}
		%\usepackage{cleveref}
	\usepackage{mathtools}
	\usepackage{siunitx}
	% these next 1 packages are temp \usepackage{relsize}

	\usepackage{algorithm}
	\usepackage{algpseudocode}

	\usepackage{graphicx}
		\graphicspath{{res/img/}}
	\usepackage{import}

	\usepackage{booktabs}

	\usepackage[citestyle=numeric,bibstyle=numeric]{biblatex}
		\addbibresource{bib/cdpr.bib}
		\addbibresource{bib/path_planning.bib}
		\addbibresource{bib/trajectory_generation.bib}
		\addbibresource{bib/math.bib}
		\nocite{*}

	\input{props/layout.tex}

	\usepackage[hidelinks]{hyperref}
	\usepackage[all]{hypcap}

	% This should be used for full thesis:
	\usepackage[section=section, acronyms]{glossaries}
	% This should be used for literature study report:
	%\usepackage[section=subsection, acronyms]{glossaries}
		\makenoidxglossaries{}
		\input{defs/notation.tex}
		\input{defs/symbols.tex}
		\input{defs/abbrevs.tex}
		\input{defs/glossary.tex}
		\input{defs/subscripts.tex}
		\setlength{\glslistdottedwidth}{4cm}
		\setglossarystyle{listdotted}
		\ifdraft{\glsaddall}{}

	% redefine sections for literature report only
	%\let\paragraph\subsubsection%
	%\let\subsubsection\subsection%
	%\let\subsection\section%
	%\let\section\chapter%

\begin{document}

	\ifdraft{\listoftodos{}}{}
%
	\pagenumbering{gobble}
	\input{chapters/title_page}
	\input{chapters/abstract}
%
	\clearpage
	\pagenumbering{roman}
%
	\tableofcontents
	\listoffigures
	\listoftables
	\listofalgorithms{}
	\input{defs/nomenclature.tex}
%
	\clearpage
	\pagenumbering{arabic}

	\part{Overview and Background}%
	\label{part:overview_and_background}

		\input{chapters/intro/intro}
		%%%%%%% \input{chapters/proposed_plan/proposed_plan}
		\input{chapters/overview/overview}
		\input{chapters/lit_study/lit_study}

	\part{Path Planning Approach}%
	\label{part:path_planning_approach}

		% Part~\ref{part:path_planning_approach} discusses the overall planning
		% approach followed. Chapter~\ref{chap:collision_detection} discusses the
		% collision detection modules. The main planning is discussed
		% in~\ref{chap:sampling}. Finally, post processing steps are discussed in
		% chapters~\ref{chap:path_processing} and~\ref{chap:motion_law}.

		\input{chapters/collision_detection/collision_detection}
		\input{chapters/sampling/sampling}
		\input{chapters/path_processing/path_processing}
		\input{chapters/motion_law/motion_law}
		% \input{chapters/experimental_validation/experimental_validation}

	\clearpage
	\pagenumbering{roman}
	\appendix
		\input{chapters/architecture_developed/architecture_developed}

	\appendix
	\printbibliography{}

\end{document}
